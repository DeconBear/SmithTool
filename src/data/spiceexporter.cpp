...
QString SpiceExporter::generateNetlist(const MatchingTrace& trace) const
{
    ...
    // MatchingTrace stores elements in load->source order (Smith chart walking direction).
    // SPICE netlists are emitted from source->load, so reverse the order here.
    const auto& segments = trace.segments();
    int elementIndex = 0;
    for (int i = static_cast<int>(segments.size()) - 1; i >= 0; --i) {
        out << formatElement(segments[static_cast<size_t>(i)], elementIndex, currentNode);
        ++elementIndex;
    }
...
QString SpiceExporter::generateSubcircuit(const MatchingTrace& trace, const QString& name) const
{
    ...
    // MatchingTrace stores elements in load->source order, reverse for source->load export.
    const auto& segments = trace.segments();
    int elementIndex = 1;
    for (auto it = segments.rbegin(); it != segments.rend(); ++it, ++elementIndex) {
        const TraceSegment& seg = *it;
        const int nextNode = currentNode + 1;
        const double value = seg.componentValue;

        QString elemName;
        if (seg.connectionType == ConnectionType::Series) {
            switch (seg.componentType) {
                case ComponentType::Resistor:
                    elemName = QString("RS%1").arg(elementIndex);
                    out << formatResistor(elemName, currentNode, nextNode, value);
                    break;
                case ComponentType::Inductor:
                    elemName = QString("LS%1").arg(elementIndex);
                    out << formatInductor(elemName, currentNode, nextNode, value);
                    break;
                case ComponentType::Capacitor:
                    elemName = QString("CS%1").arg(elementIndex);
                    out << formatCapacitor(elemName, currentNode, nextNode, value);
                    break;
                default:
                    break;
            }
        ...
        switch (seg.componentType) {
            case ComponentType::Resistor:
                elemName = QString("RP%1").arg(elementIndex);
                out << formatResistor(elemName, currentNode, 0, value);
                break;
            case ComponentType::Inductor:
                elemName = QString("LP%1").arg(elementIndex);
                out << formatInductor(elemName, currentNode, 0, value);
                break;
            case ComponentType::Capacitor:
                elemName = QString("CP%1").arg(elementIndex);
