...
void ComponentEditDialog::setComponent(ComponentType type, ConnectionType connection, double value)
{
    ...
    switch (type) {
        case ComponentType::Resistor:
            m_unitCombo->addItems({tr("Ohm"), tr("kOhm"), tr("mOhm")});
            break;
        case ComponentType::Inductor:
            m_unitCombo->addItems({tr("H"), tr("mH"), tr("uH"), tr("nH"), tr("pH")});
            m_unitCombo->setCurrentIndex(3);  // Default nH
            break;
        case ComponentType::Capacitor:
            m_unitCombo->addItems({tr("F"), tr("mF"), tr("uF"), tr("nF"), tr("pF"), tr("fF")});
...
double ComponentEditDialog::getScaledValue() const
{
    ...
    switch (m_type) {
        case ComponentType::Resistor:
            switch (unitIndex) {
                case 0: return value;           // Ohm
                case 1: return value * 1000;    // kOhm
                case 2: return value / 1000;    // mOhm
                default: break;
            }
            break;
        case ComponentType::Inductor:
...
void ComponentEditDialog::updateDisplay()
{
    ...
    switch (m_type) {
        case ComponentType::Resistor:
            impedanceText = tr("Impedance: %1 Ohm").arg(value, 0, 'f', 2);
            break;
        case ComponentType::Inductor: {
            double reactance = omega * value;
            impedanceText = tr("Reactance: j%1 Ohm (at %2 GHz)")
                .arg(reactance, 0, 'f', 2)
