...
void ComponentPanel::setupUI()
{
    ...
    inputLayout->addRow(tr("Z0 (Ohm):"), m_z0Edit);

    mainLayout->addWidget(inputGroup);

    // Gamma display
    QGroupBox* gammaGroup = new QGroupBox(tr("Reflection Coefficient Gamma"));
    QFormLayout* gammaLayout = new QFormLayout(gammaGroup);

    m_gammaRectLabel = new QLabel("0.000 + j0.000");
    m_gammaPolarLabel = new QLabel("|Gamma| = 0.000  angle 0.0 deg");
    ...
    m_zActualLabel = new QLabel("50.00 + j0.00 Ohm");
    ...
    m_resistanceLabel = new QLabel("R = 50.00 Ohm");
    m_reactanceLabel = new QLabel("X = 0.00 Ohm");
    ...
    m_returnLossLabel = new QLabel("RL = -inf dB");
...
void ComponentPanel::updateDisplay()
{
    ...
    m_gammaPolarLabel->setText(tr("|Gamma| = %1  angle %2 deg")
        .arg(gammaMag, 0, 'f', 4)
    ...
    m_zActualLabel->setText(tr("%1 %2 j%3 Ohm")
    ...
    m_resistanceLabel->setText(tr("R = %1").arg(formatValue(r, "Ohm")));
    m_reactanceLabel->setText(tr("X = %1%2")
        .arg(x >= 0 ? "+" : "")
        .arg(formatValue(x, "Ohm")));
    ...
    if (std::abs(x) < 0.01) {
        ...
        m_componentLabel->setText(tr("L = %1 (series)").arg(cv.valueWithUnit()));
    } else {
        // Capacitive
        double c = -1.0 / (2.0 * SmithMath::PI * m_frequency * x);
        ComponentValue cv(ComponentType::Capacitor, c, m_frequency);
        m_componentLabel->setText(tr("C = %1 (series)").arg(cv.valueWithUnit()));
    }
    ...
    if (vswr > 1e5) {
        m_vswrLabel->setText(tr("VSWR = inf"));
    } else {
        m_vswrLabel->setText(tr("VSWR = %1").arg(vswr, 0, 'f', 2));
    }

    if (rl < -100) {
        m_returnLossLabel->setText(tr("RL = -inf dB"));
    } else {
        m_returnLossLabel->setText(tr("RL = %1 dB").arg(rl, 0, 'f', 2));
    }
}
...
QString ComponentPanel::formatValue(double val, const QString& unit) const
{
    ...
    if (unit == "Ohm") {
        if (absVal >= 1e6) {
            return QString("%1 MOhm").arg(val / 1e6, 0, 'f', 2);
        }
        if (absVal >= 1e3) {
            return QString("%1 kOhm").arg(val / 1e3, 0, 'f', 2);
        }
        if (absVal >= 1) {
            return QString("%1 Ohm").arg(val, 0, 'f', 2);
        }
        return QString("%1 mOhm").arg(val * 1e3, 0, 'f', 2);
    }
